...
class Departement
{
    private:
   	 ...
   	 int prixparMcarre;
    public:
        Departement(int n , int p ): numero_{n}, prixparMcarre_{b};
        //Departement(json data);
        Departement(int id){
    		string url = "http://localhost:8000/departement/" + to_string(id);
    		cpr::Response r = cpr::Get(cpr::Url{url});
		json data = json::parse(r.text);
   		set(data["numero"], data["prixMcarre"]);
	};
        void set(int num, int p)
	{
 	    _nom = num;
 	   _prix = p;
	}
        ...
        void affichage ()
{
    ...
    cout << "Prix du metre carre : "<< _prixparMcarre << endl;
}
        friend class Prix;
        friend class Usine;
};
...
class QuantiteIngredient
{
    ...
    public:
        ...
        QuantiteIngredient(int id){
    		string url = "http://localhost:8000//" + to_string(id);
    		cpr::Response r = cpr::Get(cpr::Url{url});
		json data = json::parse(r.text);
   		set(data["nom"], data["prix"]);
};
...
class Usine
{
    ...
    public:
        ...
        Usine::Usine(int id) {
    // Construct the URL for the API endpoint based on the given id
    string url = "http://localhost:8000/usine/" + to_string(id);

    // Perform an HTTP GET request to the specified URL using the cpr library
    cpr::Response r = cpr::Get(cpr::Url{url});

    // Parse the response text (assumed to be in JSON format) into a JSON object
    json data = json::parse(r.text);

    // Call the set method of the Usine class to populate its attributes with the data from the API response
